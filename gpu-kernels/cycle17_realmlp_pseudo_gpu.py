"""Experiment 064 (cycle 17) — pseudo-labeled RealMLP (single-seed feasibility).
...
Single-seed feasibility (SEED=42): compare to the existing single-seed RealMLP
(realmlp_seed42 OOF 0.95355) and the 6-seed base (0.95383). If pseudo lifts the
single seed materially, escalate to multi-seed.
...
Outputs: oof_realmlp_pseudo.parquet, submission_realmlp_pseudo.csv
"""
...
OOF_OUT = WORKING / "oof_realmlp_pseudo.parquet"
SUB_OUT = WORKING / "submission_realmlp_pseudo.csv"
...
def main():
    ...
    for fold, ((tr, va), (etr, _)) in enumerate(zip(kf.split(X, strat), kfe), 1):
        t1 = time.time()
        ...
        X_va, y_va = X.iloc[va].copy(), y.iloc[va]
        te = TargetEncoder(cv=N_SPLITS, smooth="auto", shuffle=True, random_state=SEED)
        tr_enc = te.fit_transform(X_tr[combo], y_tr); va_enc = te.transform(X_va[combo]); ts_enc = te.transform(Xt[combo])
        names = [f"_{c}TE" for c in combo]
        X_tr[names] = tr_enc; X_va[names] = va_enc
        X_ts = Xt.copy(); X_ts[names] = ts_enc
        m = RealMLP_TD_Classifier(**REALMLP_PARAMS)
        m.fit(X_tr, y_tr, X_va, y_va)
        oof[va] = m.predict_proba(X_va)[:, 1]
        test_preds += m.predict_proba(X_ts)[:, 1] / N_SPLITS
        a = roc_auc_score(y_va, oof[va]); fa.append(a)
        print(f"fold {fold}/{N_SPLITS} AUC={a:.5f} train_rows={len(X_tr):,} ({time.time()-t1:.0f}s)", flush=True)

    oa = roc_auc_score(y, oof)
    print(f"\nOOF AUC: {oa:.5f}  (vs single-seed RealMLP 0.95355, 6-seed 0.95383)")
