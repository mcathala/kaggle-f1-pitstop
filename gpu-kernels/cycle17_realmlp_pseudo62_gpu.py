"""Experiment 069 (cycle 17) — round-2 pseudo-RealMLP-6seed (strong-blend labeler).
...
exp 065's pseudo-labels came from a quick XGB (~0.94). exp 067 showed that for
the XGB base, a stronger labeler (the 0.95433 blend) gives a SMALLER honest lift —
revealing round-1's OOF advantage was partly leakage. This kernel runs the
analogous round-2 for RealMLP: same 6-seed setup, but pseudo-labels are read from
the private labeler dataset `mcathala/f1-pitstop-blend-labeler` (the strong-blend
test predictions, OOF 0.95433). Expected: smaller-but-honest lift on RM (the
leakage component is removed).
...
Outputs: oof_realmlp_pseudo62.parquet, submission_realmlp_pseudo62.csv
"""
...
OOF_OUT = WORKING / "oof_realmlp_pseudo62.parquet"
SUB_OUT = WORKING / "submission_realmlp_pseudo62.csv"
...
def main():
    ...
    # ---- pseudo-labels from STRONG-BLEND labeler (round-2; replaces gen_pseudo) ----
    print("Reading strong-blend labeler (round-2 self-training)...")
