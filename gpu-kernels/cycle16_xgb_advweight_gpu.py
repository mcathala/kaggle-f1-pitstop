"""Experiment 057 (cycle 16) — adversarial-importance-weighted external data on XGB-highbins.

We measured that the external augmentation set, while it HELPS (NN ablation: removing
it costs ~-0.0008), is distribution-shifted from the competition data (adversarial
train-vs-external AUC 0.78, pos rate 0.255 vs 0.199, skewed to late-race high-pit
laps). Hypothesis: keep external's volume benefit while cutting its shift drag by
down-weighting the external rows that look LEAST like competition data.

Covariate-shift correction: train an adversarial classifier (competition=0 vs
external=1), get p = P(external|x) for each external row, and weight it by
w = clip((1-p)/p, 0.1, 10) — the textbook importance ratio P(comp|x)/P(ext|x).
Competition rows keep weight 1.0. Base recipe is verbatim cycle-11 XGB-highbins;
the ONLY change is the external sample weights.

Runs on the P100 via device=cuda.

Inputs (add in Kaggle): competition playground-series-s6e5 + external dataset.
Outputs (/kaggle/working/): oof_xgb_advweight.parquet, submission_xgb_advweight.csv
"""
...
OOF_OUT = WORKING / "oof_xgb_advweight.parquet"
SUB_OUT = WORKING / "submission_xgb_advweight.csv"
...
def main():
    ...
    # ---- adversarial importance weights for external rows (covariate-shift correction) ----
    print("Computing adversarial importance weights for external rows...")
    adv_num = [c for c in ["LapNumber", "Stint", "TyreLife", "Position", "LapTime (s)",
                           "LapTime_Delta", "Cumulative_Degradation", "RaceProgress",
                           "Position_Change", "PitStop", "Year"] if c in train.columns and c in ext.columns]
    A = pd.concat([train[adv_num], ext[adv_num]], ignore_index=True).fillna(-999.0)
    Ay = np.concatenate([np.zeros(len(train)), np.ones(len(ext))])
    p_ext_all = np.zeros(len(A))
    akf = StratifiedKFold(n_splits=4, shuffle=True, random_state=SEED)
    for atr, ava in akf.split(A, Ay):
        adm = xgb.train(
            {"objective": "binary:logistic", "eval_metric": "auc", "tree_method": "hist",
             "device": "cuda", "max_depth": 6, "eta": 0.1, "verbosity": 0},
            xgb.DMatrix(A.iloc[atr], label=Ay[atr]), num_boost_round=150)
        p_ext_all[ava] = adm.predict(xgb.DMatrix(A.iloc[ava]))
    adv_auc = roc_auc_score(Ay, p_ext_all)
    p_ext = np.clip(p_ext_all[len(train):], 1e-3, 1 - 1e-3)   # external rows only
    ext_w = np.clip((1.0 - p_ext) / p_ext, 0.1, 10.0).astype(np.float64)
    print(f"  adversarial OOF AUC={adv_auc:.4f}  ext weight: mean={ext_w.mean():.3f} "
          f"median={np.median(ext_w):.3f} min={ext_w.min():.3f} max={ext_w.max():.3f}")
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X, strat_key), start=1):
        ...
        w_tr = np.concatenate([np.ones(len(tr_idx)), ext_w])   # comp rows 1.0, ext rows importance-weighted
