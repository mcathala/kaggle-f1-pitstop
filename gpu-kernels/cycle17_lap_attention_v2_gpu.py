"""Experiment 059 (cycle 17) — STRENGTHENED lap-attention on Kaggle GPU.

exp 058 established that the lap-attention architecture (a Transformer self-
attending over each driver-race's sampled laps) is the joint-most-diverse base
in the project (rho 0.90 vs every base) but too weak in minimal form (OOF 0.936,
below the 0.949 floor). Its diversity source is CROSS-LAP ATTENTION, not numeric
embeddings — so, unlike embMLP-v2 (exp 054, whose diversity collapsed rho
0.908->0.978 when strengthened because its strength = RealMLP's embedding
mechanism), strengthening lap-attention might raise AUC while KEEPING rho low.

This run adds the three levers that got embMLP 0.941->0.951, without changing the
architecture:
  1. EXTERNAL sequences  — group f1_strategy_dataset into driver-race seqs,
                           add as always-train groups (exp 056: +~0.0009 for NNs).
  2. DOMAIN FE numerics  — the same engineered per-lap features the trees use
                           (ratios, pressures, remaining-laps, etc.) + 4 binned
                           categoricals as extra embeddings.
  3. 3-SEED ensemble     — average OOF + test over seeds {42, 7, 99}.

Hypothesis: OOF clears 0.949 AND rho vs RealMLP stays < 0.97 -> first base in the
empty "strong + diverse" quadrant -> lifts the 3-way blend (0.95421) by >=+0.00020.
...
Outputs (/kaggle/working/): oof_lap_attention_v2.parquet, submission_lap_attention_v2.csv
"""
...
OOF_OUT = WORKING / "oof_lap_attention_v2.parquet"
SUB_OUT = WORKING / "submission_lap_attention_v2.csv"
...
NUM_COLS = BASE_NUM + DOMAIN_NUM
CAT_COLS = BASE_CAT + BIN_CAT
