"""Experiment 067 (cycle 17) — round-2 self-training: use the strong-blend labeler.

exp 063's labeler was a quick XGB (~0.94). exp 065's strong-blend reaches OOF
0.95433 — a much higher-quality labeler. Round-2 self-training uses these stronger
labels: more accurate confident pseudo-labels -> potentially a bigger XGB lift
than exp 063's +0.00032 standalone. Labeler test predictions are loaded from the
private Kaggle dataset `mcathala/f1-pitstop-blend-labeler` (labeler_blend.csv).

  Gate   — confident test rows: blend_prob >= 0.92 -> 1, <= 0.03 -> 0.
  Pass 2 — verbatim cycle-11 XGB-highbins on comp + external + pseudo-test, 5-fold;
           OOF measured on competition rows only (pseudo-test never in val).

Hypothesis: stronger labels -> XGB OOF above exp 063's 0.95295 AND blend above
exp 065's 0.95433.

Inputs: competition playground-series-s6e5 + external dataset.
Outputs: oof_xgb_pseudo2.parquet, submission_xgb_pseudo2.csv
"""
...
OOF_OUT = WORKING / "oof_xgb_pseudo2.parquet"
SUB_OUT = WORKING / "submission_xgb_pseudo2.csv"
...
def main():
    ...
    # ---- Read strong-blend labeler from private dataset (round-2 self-training) ----
    print("Reading strong-blend labeler (OOF 0.95433)...")
    lab_path = find_one("labeler_blend.csv")
    lab = pd.read_csv(lab_path).sort_values(ID_COL).reset_index(drop=True)
    test_sorted = test.sort_values(ID_COL).reset_index(drop=True)
    assert (lab[ID_COL].to_numpy() == test_sorted[ID_COL].to_numpy()).all(), "labeler id mismatch"
    # align labeler predictions to test row order
    id_to_p = dict(zip(lab[ID_COL].to_numpy(), lab[TARGET].to_numpy()))
    tp1 = np.array([id_to_p[i] for i in test[ID_COL].to_numpy()])
    ...
    dt=xgb.DMatrix(Xt, enable_categorical=True)   # test DMatrix (no longer built by pass-1)
