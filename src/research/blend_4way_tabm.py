"""Experiment 049 (cycle 16) — 4-way blend probe with TabM as 4th base.
...
This probe asks whether adding TabM_D_Classifier (cycle 16 GPU run) as a 4th
base lifts blend OOF AUC by ≥ +0.00020 (clearing 0.95441).

Two-stage sweep:
  Stage 1 — 1D: w_tabm sweep, other weights rescaled to preserve cycle-11 ratios.
                Tells us "is TabM useful at all in the blend?"
  Stage 2 — 3D: (w_cb, w_xgb, w_tabm) grid; w_realmlp = 1 - sum.
                Only runs if Stage 1 best has w_tabm > 0.

Outputs:
  data/blend_4way_tabm_sweep.parquet  — sweep results
  data/oof_blend_4way_tabm.parquet    — best 4-way OOF (only if hurdle cleared)
  data/submission_blend_4way_tabm.csv — best 4-way submission (only if hurdle cleared)
"""
...
TABM_OOF = DATA / "oof_tabm_kaggle.parquet"
...
TABM_SUB = DATA / "submission_tabm_kaggle.csv"

SWEEP_OUT = DATA / "blend_4way_tabm_sweep.parquet"
OOF_OUT = DATA / "oof_blend_4way_tabm.parquet"
SUB_OUT = DATA / "submission_blend_4way_tabm.csv"
...
def main() -> None:
    ...
    for p in (RM_OOF, CB_OOF, XGB_OOF, TABM_OOF):
        if not p.exists():
    ...
    print(f"  TabM:      OOF AUC = {roc_auc_score(y, p_tabm):.5f}")

    # Rank-correlation matrix — diagnostic for blend value
    rs = pd.DataFrame({"realmlp": p_rm, "cb": p_cb, "xgb": p_xgb, "tabm": p_tabm}).rank()
    ...
    # ===== Stage 1: 1D w_tabm sweep, rescale others to cycle-11 ratios =====
    print(f"\nStage 1 — w_tabm sweep (other weights rescaled to cycle-11 ratios)")
    print(f"  {'w_tabm':>7}  {'w_rm':>6}  {'w_cb':>6}  {'w_xgb':>6}  {'AUC':>8}  {'Δ vs c11':>10}")
    print(f"  {'-'*7}  {'-'*6}  {'-'*6}  {'-'*6}  {'-'*8}  {'-'*10}")

    rows = []
    rest = 1.0 - 0.0  # placeholder
    for w_tabm in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50]:
        rest = 1.0 - w_tabm
        w_rm = CYCLE11_W["realmlp"] * rest
        w_cb = CYCLE11_W["cb"] * rest
        w_xgb = CYCLE11_W["xgb"] * rest
        p = w_rm * p_rm + w_cb * p_cb + w_xgb * p_xgb + w_tabm * p_tabm
        auc = roc_auc_score(y, p)
        delta = auc - CYCLE11_OOF
        rows.append({
            "stage": "1d", "w_realmlp": w_rm, "w_cb": w_cb, "w_xgb": w_xgb, "w_tabm": w_tabm,
            "auc": auc, "delta_vs_c11": delta,
        })
        print(f"  {w_tabm:7.3f}  {w_rm:6.3f}  {w_cb:6.3f}  {w_xgb:6.3f}  {auc:8.5f}  {delta:+10.5f}")

    df = pd.DataFrame(rows)
    best_s1 = df.loc[df["auc"].idxmax()]
    print(f"\nStage 1 best: w_tabm={best_s1['w_tabm']:.3f}  AUC={best_s1['auc']:.5f}  "
          f"Δ={best_s1['delta_vs_c11']:+.5f}")

    # If Stage 1 picks w_tabm = 0, blend won't benefit from TabM. Stop.
    if best_s1["w_tabm"] == 0.0:
        print("\n✗ Stage 1 optimum is w_tabm=0. TabM does NOT earn blend weight.")
    ...
    w_tabm0 = float(best_s1["w_tabm"])
    cb0 = CYCLE11_W["cb"]
    xgb0 = CYCLE11_W["xgb"]
    print(f"\nStage 2 — 3D grid (w_cb, w_xgb, w_tabm), w_rm = 1 - sum")

    fine_rows = []
    for w_tabm in np.arange(max(0.0, w_tabm0 - 0.10), min(0.5, w_tabm0 + 0.101), 0.025):
        for w_cb in np.arange(0.00, 0.151, 0.025):
            for w_xgb in np.arange(max(0.05, xgb0 - 0.10), min(0.45, xgb0 + 0.101), 0.025):
                w_rm = 1.0 - w_cb - w_xgb - w_tabm
                if w_rm <= 0:
                    continue
                p = w_rm * p_rm + w_cb * p_cb + w_xgb * p_xgb + w_tabm * p_tabm
                auc = roc_auc_score(y, p)
                ...
                fine_rows.append({
                    ...
                    "w_xgb": float(w_xgb), "w_tabm": float(w_tabm),
                    "auc": auc, "delta_vs_c11": delta,
                })
    ...
    print(f"\nBest overall: w_rm={best['w_realmlp']:.4f}  w_cb={best['w_cb']:.4f}  "
          f"w_xgb={best['w_xgb']:.4f}  w_tabm={best['w_tabm']:.4f}")
    ...
    if best["auc"] >= HURDLE:
        ...
        wm, wc, wx, wt = best["w_realmlp"], best["w_cb"], best["w_xgb"], best["w_tabm"]
        pd.DataFrame({
            "id": rm.index,
            "Year": rm["Year"].values,
            "target": y,
            "oof": wm * p_rm + wc * p_cb + wx * p_xgb + wt * p_tabm,
        }).to_parquet(OOF_OUT, index=False)
        print(f"Wrote {OOF_OUT.name}")

        for path in (RM_SUB, CB_SUB, XGB_SUB, TABM_SUB):
            if not path.exists():
        ...
        st = pd.read_csv(TABM_SUB).sort_values(ID_COL).reset_index(drop=True)
        assert (sm[ID_COL] == sc[ID_COL]).all(), "id mismatch RM vs CB"
        assert (sm[ID_COL] == sx[ID_COL]).all(), "id mismatch RM vs XGB"
        assert (sm[ID_COL] == st[ID_COL]).all(), "id mismatch RM vs TabM"
        ...
        print(f"\nSubmit with: .venv/bin/kaggle competitions submit -c playground-series-s6e5 "
              ...
              f"XGB={wx:.3f} TabM={wt:.3f} OOF={best['auc']:.5f}'")
        ...
        print(f"\n✗ 4-way did NOT beat cycle 11. TabM adds no blend value.")
        print("  Closing 4-way axis at cycle-11 baseline. No submission.")
