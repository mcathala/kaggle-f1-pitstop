"""Experiment 091 — 2022-upweighted diffFE-XGB (target the weakest within-year slice).

Diagnostic (cycle 18): the pooled OOF 0.95462 is inflated by between-year
separation (2023 pits 1% vs ~28% elsewhere — nearly-free ranking). The competitive
limit is WITHIN-year skill, and 2022 is by far the weakest (within-year AUC 0.920
vs 0.935–0.953 elsewhere) — likely the 2022 ground-effect regulation change shifted
tyre/pit dynamics our features fit worse. This base is identical to diffFE-XGB
EXCEPT 2022 rows (train + ext) get sample_weight=1.6, forcing the splitter to fit
2022 harder. Gate: pooled OOF ≥ diffFE-XGB 0.95291 AND 2022 within-year AUC up,
without tanking other years. (Risk: distorts cross-year calibration → pooled flat.)

Outputs:
  data/oof_xgb_y2022up.parquet
  data/submission_xgb_y2022up.csv
"""
...
OOF_OUT = DATA / "oof_xgb_y2022up.parquet"
SUB_OUT = DATA / "submission_xgb_y2022up.csv"
WEIGHT_2022 = 1.6  # sample-weight for 2022 rows (the weakest within-year slice, AUC 0.920)
...
def main() -> None:
    ...
    add_frequency_features([train, test, ext], BASE_CATS)  # diffFE: only base-cat freq, no cross/bins
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X, strat_key), start=1):
        ...
        # exp 091: upweight 2022 rows (weakest within-year slice) in train + ext.
        yr_tr = np.concatenate([train["Year"].to_numpy()[tr_idx], ext["Year"].to_numpy()])
        w_tr = np.where(yr_tr == 2022, WEIGHT_2022, 1.0).astype("float32")
