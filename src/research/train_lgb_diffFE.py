"""Experiment 046 (cycle 13) — LightGBM with max_bin=5000.

Cycle 11 exp 044 confirmed that XGBoost's standalone AUC jumps from 0.946 to 0.953
once max_bin lifts from 256 to 5000, motivated by cycle-10 probe 1's finding that
Race_Year (8.83) and Race_Compound_Stint (4.93) carry top mean importance.

Open question: is `max_bin` the universal cat-cardinality fix, or XGBoost-specific?
LightGBM has the same parameter (default 255) and uses categorical-split logic that
differs from XGBoost (leaf-wise growth vs level-wise; voting splits vs partitioning).
If LGB-highbins reaches standalone ≥0.949 AND keeps rank-corr <0.97 vs RM/CB/XGB,
it becomes a viable 4th blend base — leaf-wise growth produces meaningfully
different rank orderings than XGBoost's level-wise histograms.

Outputs:
  data/oof_lgb_highbins.parquet
  data/submission_lgb_highbins.csv
"""
...
OOF_OUT = DATA / "oof_lgb_diffFE.parquet"
SUB_OUT = DATA / "submission_lgb_diffFE.csv"
...
# LightGBM HPs adapted for the high-cardinality cross-cats finding (probe 1).
# Key choices analogous to XGB-highbins (exp 044):
#   max_bin: 255 → 5000           (LightGBM equivalent of XGB's max_bin fix)
#   num_leaves: 31 → 127           (LGB uses leaves not depth; 127 ≈ depth 10 capacity)
#   learning_rate: 0.05 → 0.01    (slow lr matches added capacity)
#   feature_fraction: 0.7 → 0.15  (heavy column subsample = XGB's colsample_bytree)
#   bagging_fraction: 0.8 → 0.85  (row subsample)
#   lambda_l1 / lambda_l2: heavy   (analogous to XGB's reg_alpha/reg_lambda)
#   min_data_in_leaf: 200          (LGB's analog to XGB's min_child_weight regime)
#   cat_smooth: 10                 (smoothing for categorical splits)
LGB_PARAMS = {
    "objective": "binary",
    "metric": "auc",
    "boosting_type": "gbdt",
    "max_bin": 5000,                # ← THE KEY FIX (LGB default 255)
    "num_leaves": 127,              # ~depth 10 capacity
    "learning_rate": 0.01,
    "feature_fraction": 0.15,       # heavy column subsample
...
def main() -> None:
    ...
    # diffFE: skip cross-categoricals

    cross_cats = [] if True else [
    ...
    add_frequency_features([train, test, ext], BASE_CATS)  # diffFE

    print("Adding group-statistic features...")
    group_stat_cols = []  # diffFE
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X, strat_key), start=1):
        ...
        dval = lgb.Dataset(X_va, label=y_va, categorical_feature=cat_idx, reference=dtrain, free_raw_data=False)
        ...
        oof[va_idx] = va_pred
        test_preds += booster.predict(X_test, num_iteration=best_iter) / N_SPLITS
        fold_auc = roc_auc_score(y_va, va_pred)
        fold_aucs.append(fold_auc)
        fold_iters.append(best_iter)
        print(
            f"fold {fold}/{N_SPLITS}  AUC={fold_auc:.5f}  iters={best_iter}  "
