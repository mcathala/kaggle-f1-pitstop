"""Experiment 034 (cycle 11) — XGBoost as 3rd diverse base.

Cycle 8 exp 023 added LightGBM as a 3rd blend base and found w_lgb=0 was
optimal (rank-corr LGB-CB 0.94, too similar for diversity). XGBoost has
different split mechanism (level-wise + histogram) and different regularization
than both LightGBM (leaf-wise) and CatBoost (ordered TE). Untested in this
project — exp 034 fills the gap.

Recipe: cycle 14's FE pipeline (132 features, 16 categoricals) applied as-is.
XGBoost reads categoricals via `enable_categorical=True` (XGBoost >= 2.0).

Outputs:
  data/oof_xgb.parquet      OOF predictions
  data/submission_xgb.csv   test predictions
"""
...
OOF_OUT = DATA / "oof_xgb_v2.parquet"
SUB_OUT = DATA / "submission_xgb_v2.csv"
...
# XGBoost HPs v2 — push standalone AUC higher with deeper trees + slower lr +
# more iterations. Exp 034 v1 (depth=8, lr=0.05, iter=3000) hit 0.94615 standalone.
# Try depth=10, lr=0.02, iter=10000 with longer early-stop.
XGB_PARAMS = {
    "objective": "binary:logistic",
    "eval_metric": "auc",
    "tree_method": "hist",
    "enable_categorical": True,
    "max_depth": 10,           # 8 → 10: more capacity per tree
    "eta": 0.02,               # 0.05 → 0.02: slower learning, more iters
    "min_child_weight": 3.0,   # 5.0 → 3.0: allow finer splits
    "subsample": 0.85,
    "colsample_bytree": 0.75,
    "reg_lambda": 3.0,         # 5.0 → 3.0: less L2 regularization
    "reg_alpha": 0.3,
    "max_cat_to_onehot": 8,    # 4 → 8: one-hot more small cats
    "max_cat_threshold": 64,   # default 64; explicit
    "nthread": -1,
    "verbosity": 1,
}
N_ROUNDS = 10000             # 3000 → 10000
EARLY_STOP = 500             # 200 → 500
