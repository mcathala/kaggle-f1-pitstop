"""Experiment 019 (cycle 6) — RealMLP + historical driver aggregates.

Identical to experiment 016 (cycle 4 RealMLP) except: BEFORE the light FE,
we compute historical aggregates per Driver / (Driver, Race) / (Driver, Year)
on the COMBINED train + test (and separately on external).

17 new features capture driver behavioral signatures from input columns only —
no labels, no PitStop (to avoid the label-proxy pattern that killed exp 018).
Driver avg LapTime / std / TyreLife / Position / Compound usage etc. give the
NN's embedding layer additional per-driver context beyond what the 6-dim
Driver embedding can capture from raw rows.

This experiment targets the driver-level residual that cycle 13's EDA flagged
as the structurally hardest. Cycle 5's audit identified the feature category
but the audit's specific suggestion (forward features) failed because it was
label-proxy; this version is a safer angle on the same goal.

Outputs:
  data/oof_realmlp_hist.parquet
  data/submission_realmlp_hist.csv
"""
...
OOF_OUT = DATA / "oof_realmlp_hist.parquet"
SUB_OUT = DATA / "submission_realmlp_hist.csv"
...
def main() -> None:
    ...
    # Historical aggregates per Driver / (Driver, Race) / (Driver, Year) computed
    # on combined train+test (no labels involved — see src/historical_features.py).
    # External data gets its own aggregates computed independently (different
    # driver distribution).
    print("Adding historical driver aggregates...")
    combined = pd.concat(
    ...
    train = (
        combined[combined["__split"] == "train"]
        .drop(columns="__split")
        .reset_index(drop=True)
    )
    test = (
        combined[combined["__split"] == "test"]
        .drop(columns=["__split", TARGET], errors="ignore")
        .reset_index(drop=True)
    )
    orig = add_historical_features(orig)
    print(f"  train: {train.shape}  test: {test.shape}  orig: {orig.shape}")
