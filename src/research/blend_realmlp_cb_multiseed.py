"""Experiment 035 (cycle 11) — multi-seed CB × RealMLP blend probe.

Replaces cycle 7's single-seed CB-tuned-exp14 (LB 0.95097) with multi-seed CB.
Sweeps w_cb across linear AND rank-blend, picks the best, writes submission only
if it clears the hurdle.

Inputs:
  - oof_realmlp_multiseed.parquet        (RealMLP × 5 seeds, anchor)
  - oof_cb_multiseed.parquet             (multi-seed CB-tuned, from combine_cb_seeds.py)
  - submission_realmlp_multiseed.csv
  - submission_cb_multiseed.csv

Hurdle: cycle 7's blend OOF was 0.95408. Need ≥ 0.95428 to be worth submitting
(min_delta 0.00020 over cycle 7).
"""
...
CB_OOF = DATA / "oof_cb_multiseed.parquet"
REALMLP_SUB = DATA / "submission_realmlp_multiseed.csv"
CB_SUB = DATA / "submission_cb_multiseed.csv"

# Outputs
SWEEP_OUT = DATA / "blend_cb_multiseed_sweep.parquet"
OOF_OUT = DATA / "oof_blend_multiseed_best.parquet"
SUB_OUT = DATA / "submission_blend_multiseed_best.csv"

TARGET = "PitNextLap"
ID_COL = "id"
WEIGHTS = [0.0, 0.05, 0.10, 0.125, 0.15, 0.175, 0.20, 0.225, 0.25, 0.275, 0.30]
