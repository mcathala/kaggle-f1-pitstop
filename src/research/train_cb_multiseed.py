"""Experiment 035 (cycle 11) — multi-seed CB-tuned (the deferred exp 015).

Pre-registered for cycle 4 as exp 015 but never ran (RealMLP took precedence).
This implements it: bit-identical recipe to cycle 14's CB-tuned-exp14, single
HP change being just the `--seed` CLI flag.

The existing `data/oof_cb_tuned_exp14.parquet` was produced at seed=42. To get
3-seed multi-seed, run this with --seed 7 and --seed 99, then average all three
OOFs.

Outputs (per invocation, per seed S):
  data/oof_cb_tuned_seed{S}.parquet      OOF predictions
  data/submission_cb_tuned_seed{S}.csv   test predictions
"""
...
N_SPLITS = 5
CV_SEED = 42  # FIXED — determines fold construction. Must match cycle 14.
# MODEL_SEED is a CLI arg — only the CB random_seed varies between multi-seed runs.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--seed", type=int, required=True, help="random seed (e.g. 7, 99)")
    return p.parse_args()
...
def main() -> None:
    ...
    OOF_OUT = DATA / f"oof_cb_tuned_seed{MODEL_SEED}.parquet"
    SUB_OUT = DATA / f"submission_cb_tuned_seed{MODEL_SEED}.csv"
    print(f"Args: model_seed={MODEL_SEED}  cv_seed={CV_SEED}  → {OOF_OUT.name}, {SUB_OUT.name}")
    ...
    # CV folds are FIXED by CV_SEED so multi-seed OOFs aggregate row-by-row.
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X, strat_key), start=1):
        ...
        params["random_seed"] = MODEL_SEED + fold  # per-fold offset for variance reduction within seed
    ...
    print(f"  (vs CB-tuned-exp14 seed42 0.95114, Δ = {oof_auc - 0.95114:+.5f})")
