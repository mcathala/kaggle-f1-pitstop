"""Experiment 051 (cycle 16) — within-stint trajectory (lag) features on XGB-highbins.

Our cycle-11 XGB-highbins base sees only the CURRENT lap. But our cycle-10 probe-2
residual EDA localised the worst-loss quartile to a coherent dynamic slice —
degraded tyre x losing position x pit-cluster — which is a *trajectory* (pace
dropping, positions slipping over the last few laps), not a static row state.
None of our bases capture that.

This experiment adds within-stint backward-lag features: for each
(Year, Race, Driver, Stint) group, the previous 1-2 laps' values of the neutral
input columns (LapTime, LapTime_Delta, Cumulative_Degradation, Position) plus the
lap-over-lap delta (current - lag1). We lag ONLY neutral inputs — never PitStop or
the target — to avoid the label-proxy regression we saw when forward features were
tried earlier. Median stint is 3 laps, so lag-1 + deltas carry most of the signal;
XGBoost handles the NaN lags (stint-start rows) natively.

Base recipe is otherwise identical to cycle-11 XGB-highbins (max_bin=5000, eta=0.01,
depth=10, heavy L1/L2, colsample=0.145) so the only moving part is the lag features.

Outputs:
  data/oof_xgb_lagfe.parquet        OOF predictions
  data/submission_xgb_lagfe.csv     test predictions
"""
...
OOF_OUT = DATA / "oof_xgb_lagfe.parquet"
SUB_OUT = DATA / "submission_xgb_lagfe.csv"
...
# Columns whose recent within-stint trajectory is informative for an imminent pit.
# Strictly neutral inputs — we never lag PitStop / the target (label-proxy risk).
LAG_COLS = ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation", "Position"]
LAG_GROUP = ["Year", "Race", "Driver", "Stint"]


def add_lag_features(df: pd.DataFrame) -> pd.DataFrame:
    """Within-stint backward lags (previous laps) + lap-over-lap deltas.

    Sorts by (group, LapNumber), shifts within each (Year,Race,Driver,Stint)
    group so each row sees only PAST laps (no future leakage), then restores the
    original row order so positional fold indexing downstream stays valid.
    Stint-start rows get NaN lags, which XGBoost routes natively.
    """
    out = df.sort_values(LAG_GROUP + ["LapNumber"]).copy()
    grp = out.groupby(LAG_GROUP, sort=False)
    for col in LAG_COLS:
        if col not in out.columns:
            continue
        lag1 = grp[col].shift(1)
        lag2 = grp[col].shift(2)
        out[f"_{col}_lag1"] = lag1.astype("float32")
        out[f"_{col}_lag2"] = lag2.astype("float32")
        out[f"_{col}_d1"] = (out[col] - lag1).astype("float32")   # lap-over-lap change
    # Laps elapsed in the current stint (explicit trajectory length signal).
    out["_laps_in_stint"] = grp.cumcount().astype("float32")
...
def main() -> None:
    ...
    print("Adding within-stint lag features...")
    train = add_lag_features(train)
    test = add_lag_features(test)
    ext = add_lag_features(ext)
    n_lag = len([c for c in train.columns if c.startswith("_") and ("_lag" in c or "_d1" in c or c == "_laps_in_stint")])
    print(f"  added {n_lag} lag/trajectory features")
