...
OOF_OUT = DATA / "oof_realmlp_isorig_s42.parquet"
SUB_OUT = DATA / "submission_realmlp_isorig_s42.csv"
...
def main():
    ...
    # is_original flag: external rows are distribution-shifted (adv-AUC ~0.78,
    # pit-rate 25.5% vs comp 19.9%). Flag lets RealMLP learn a distinct response
    # for the shifted distribution; all comp/test/pseudo rows are 0, so at
    # inference the model uses the competition-distribution behaviour. RealMLP.fit
    # has no sample_weight, so the feature flag is our reweighting mechanism.
    import numpy as _np
    X["is_original"] = _np.float32(0.0)
    Xt["is_original"] = _np.float32(0.0)
    Xe["is_original"] = _np.float32(1.0)
