...
Outputs:
  data/oof_ftt.parquet
  data/submission_ftt.csv
"""
...
OOF_OUT = DATA / "oof_ftt.parquet"
SUB_OUT = DATA / "submission_ftt.csv"
...
# FTT_D_Classifier (FT-Transformer with tuned defaults). Only override the
# essentials — let PyTabKit's pre-tuned defaults handle attention dims, dropout,
# initialization, etc. None means "use the paper's pre-tuned value".
REALMLP_PARAMS = {
    "random_state": SEED,
    "verbosity": 2,
    "device": device,
...
def main() -> None:
    ...
    for fold, ((tr_idx, va_idx), (or_tr_idx, or_va_idx)) in enumerate(
        ...
        model = FTT_D_Classifier(**REALMLP_PARAMS)
