"""Experiment 090 — fully-inductive diffFE-XGB (structural-drift test).

A1 (nested-CV) proved our blend OOF is honest (+0.00001 weight-overfit), so the
−0.0006 OOF→LB drift is structural: transductive FE — base-cat frequency computed
on the train+TEST+ext union — inflates train OOF without a matching test boost.
RealMLP's FE is already train-only (small standalone drift, exp 079); the leak
lives in the GBDT bases' union-frequency. This base is identical to diffFE-XGB
EXCEPT base-cat frequency is counted on [train, ext] only (test excluded).

Expect LOWER OOF (loses the transductive boost) but possibly HIGHER LB / smaller
drift. Can't LB-validate offline — staged as a reset-submission candidate; the
real test is whether an inductive-XGB blend beats the standard blend on LB.

Outputs:
  data/oof_xgb_inductive.parquet
  data/submission_xgb_inductive.csv
"""
...
OOF_OUT = DATA / "oof_xgb_inductive.parquet"
SUB_OUT = DATA / "submission_xgb_inductive.csv"
...
def add_frequency_features(frames: list[pd.DataFrame], cat_cols: list[str],
                           count_frames: list[pd.DataFrame] | None = None) -> None:
    """In-place: add `<col>_count` and `<col>_freq` to every frame.

    Counts are estimated on `count_frames` (default: all frames). The INDUCTIVE
    variant (exp 090) passes count_frames=[train, ext] to EXCLUDE test from the
    frequency estimate — testing whether the transductive (test-in-union)
    frequency is the source of our structural OOF→LB drift (A1 finding)."""
    if not cat_cols:
        return
    cframes = count_frames if count_frames is not None else frames
    total = sum(len(f) for f in cframes)
    for col in cat_cols:
        if not all(col in f.columns for f in frames):
            continue
        union = pd.concat([f[col].astype("string").fillna("__NA__") for f in cframes], axis=0)
...
def main() -> None:
    ...
    # diffFE: skip cross-categoricals entirely (decorrelate from rich-FE XGB)

    # DIFFERENTIATED FE (exp 080): drop cross-cats to decorrelate from rich-FE XGB.
    cross_cats = []
    ...
    # INDUCTIVE (exp 090): count on [train, ext] only — exclude test from the freq union.
    add_frequency_features([train, test, ext], BASE_CATS, count_frames=[train, ext])

    print("Adding group-statistic features...")
    group_stat_cols = []  # diffFE: drop group-stats entirely
