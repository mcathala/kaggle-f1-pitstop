"""Experiment 089 (cycle 18) — tyre-overdue FE on RealMLP (raise ABSOLUTE strength).

exp 088 closed the diverse-base lever (even ρ 0.959 @ 0.952 adds 0 to the
RealMLP-dominated blend; the hurdle is ρ≤0.92, unreachable at strength). The only
path left to lift a saturated blend is raising the ABSOLUTE strength of the
dominant base. Pit decisions are tyre-cliff driven, and our one untried-on-RM
mechanism-aligned real feature is compound-relative tyre-overdue (exp 052 tried it
on XGB → rank-invariant; never on RealMLP). Compound-level TyreLife quantiles are
computed on TRAIN ONLY (inductive → drift-safe), then TyreLife is expressed
relative to its compound's typical pit window.

This is a clean FE ABLATION at fixed altHP hyperparameters: same recipe as
train_realmlp_altHP.py (baseline single-seed OOF 0.95370), ONLY the tyre-overdue
features added. Any OOF delta is therefore purely the feature's effect — real
features, no prediction-stacking (avoids the exp 075 OOF-mirage trap).

Gate: single-seed OOF ≥ 0.9542 (a real lift over the 0.95370 altHP baseline → the
feature helps RealMLP, worth porting to the canonical 6-seed recipe). Neutral
(≈0.9537) → tyre FE is rank-invariant for RealMLP too; FE-for-RM closed.

  - device='mps' (M1); SEEDS = [42] (single-seed ablation)
"""
...
OOF_OUT = DATA / "oof_realmlp_tyrefe_s42.parquet"
SUB_OUT = DATA / "submission_realmlp_tyrefe_s42.csv"
...
def feature_engineering(df, fit, state):
    ...
    # exp 089: compound-relative tyre-overdue. TyreLife vs its compound's typical
    # pit window. Quantiles fit on TRAIN ONLY (inductive → drift-safe); unseen
    # compounds fall back to the global quantile.
    if "Compound" in df.columns and "TyreLife" in df.columns:
        if fit:
            q = df.groupby("Compound")["TyreLife"].quantile([0.5, 0.75, 0.9]).unstack()
            gq = df["TyreLife"].quantile([0.5, 0.75, 0.9])
            state["_tyre_q"] = q
            state["_tyre_gq"] = gq
        q = state["_tyre_q"]; gq = state["_tyre_gq"]
        cmp = df["Compound"].astype(str)
        p50 = cmp.map(q[0.5]).fillna(gq[0.5]).astype("float32")
        p75 = cmp.map(q[0.75]).fillna(gq[0.75]).astype("float32")
        p90 = cmp.map(q[0.9]).fillna(gq[0.9]).astype("float32")
        tl = df["TyreLife"].astype("float32")
        df["_tyre_overdue_p75"] = (tl - p75).astype("float32")
        df["_tyre_overdue_p90"] = (tl - p90).astype("float32")
        df["_tyre_life_ratio_p50"] = (tl / (p50 + 1e-6)).astype("float32")
        df["_tyre_over_p90_flag"] = (tl > p90).astype("int8")
...
def main():
    ...
    print(f"\n{len(SEEDS)}-seed OOF AUC: {oa:.5f}  (vs altHP baseline 0.95370, Δ = {oa - 0.95370:+.5f}; gate ≥0.9542)")
