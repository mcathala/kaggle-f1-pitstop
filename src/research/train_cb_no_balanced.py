"""Experiment 032 (cycle 10) — CB-tuned-exp14 minus `auto_class_weights="Balanced"`.

Single HP change vs cycle 14's CB-tuned-exp14:
  - `auto_class_weights="Balanced"` removed → CatBoost uses natural class distribution.
  - iter 8000 → 6000, early_stopping_rounds 500 → 400 (no-Balanced expected to
    converge faster; the cycle-14 trace shows convergence in the 3-5k iter range
    anyway, so the new cap is not load-bearing).

Rationale from probe 5 (exp 030): cycle-7 blend over-predicts positives in bins
7-9 (max bias +0.057 at bin 8). The structural cause is `Balanced` weighting
applied to a ~21%-positive dataset, which implicitly upweights positives ~3.8x
and pulls mid-confidence predictions toward positive. Removing it should restore
gradient attention to the hard-example slice that probe 2 flagged (Q4 worst-loss
is 46.5% positives).

Everything else (FE, CV, seed, external data) is bit-identical to exp 014.

Outputs:
  data/oof_cb_no_balanced.parquet      OOF predictions
  data/submission_cb_no_balanced.csv   test predictions
"""
...
OOF_OUT = DATA / "oof_cb_no_balanced.parquet"
SUB_OUT = DATA / "submission_cb_no_balanced.csv"
...
# Experiment 032: drop auto_class_weights="Balanced"; iter cap 8000 → 6000,
# early_stopping 500 → 400. All other HPs identical to experiment 014.
CB_PARAMS = {
    "iterations": 6000,
    "learning_rate": 0.018,
    "depth": 8,
    ...
    "eval_metric": "AUC",
    # auto_class_weights INTENTIONALLY removed (was "Balanced" in exp 014).
...
def main() -> None:
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X, strat_key), start=1):
        ...
        # Dry-run diagnostics: rank-corr with RealMLP-multiseed + calibration bias on bin 8.
        if dry_run and REALMLP_OOF.exists():
            try:
                realmlp_oof = pd.read_parquet(REALMLP_OOF)
                rm_map = dict(zip(realmlp_oof["id"], realmlp_oof["oof"]))
                rm_va = train.iloc[va_idx][ID_COL].map(rm_map).to_numpy()
                if not np.isnan(rm_va).any():
                    rho, _ = spearmanr(va_pred, rm_va)
                    print(f"  fold {fold} rank-corr-with-RealMLP = {rho:.5f}", flush=True)
            except Exception as e:
                print(f"  fold {fold} rank-corr skipped: {e}", flush=True)
            try:
                order = np.argsort(va_pred)
                n = len(va_pred)
                bin_size = n // 10
                start = 7 * bin_size
                end = start + bin_size
                bin_idx = order[start:end]
                pred_mean = va_pred[bin_idx].mean()
                actual_rate = y_va[bin_idx].mean()
                bias = pred_mean - actual_rate
                print(f"  fold {fold} bin-8 calibration: pred_mean={pred_mean:.4f}  "
                      f"actual_rate={actual_rate:.4f}  bias={bias:+.4f}  "
                      f"(probe-5 baseline +0.0573)", flush=True)
            except Exception as e:
                print(f"  fold {fold} calibration check skipped: {e}", flush=True)
    ...
    print(f"  (vs cycle-10 pass gate 0.95150, Δ = {oof_auc - 0.95150:+.5f})")
