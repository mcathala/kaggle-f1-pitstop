...
OOF_OUT = DATA / "oof_xgb_normtyre.parquet"
SUB_OUT = DATA / "submission_xgb_normtyre.csv"
...
def add_domain_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Compound-stint-normalized tyre age: how far through a TYPICAL stint for this
    # compound is this tyre? Our analysis of stint structure (max TyreLife per
    # (Driver,Race,Year,Stint), median across stints) gives a per-compound stint
    # length; normalizing TyreLife by it yields a pit-overdue signal that crosses
    # 1.0 when the tyre is past its compound-typical life. Distinct from
    # TyreAgeVsRace (race-length-normalized) which ignores compound.
    _comp_stint = {"HARD": 23.0, "INTERMEDIATE": 16.0, "MEDIUM": 17.0, "SOFT": 14.0, "WET": 14.0}
    _cms = out["Compound"].map(_comp_stint).fillna(17.0).astype("float32")
    out["NormTyreLife"] = (out["TyreLife"] / _cms).astype("float32")
    out["LapsOverdue"] = (out["TyreLife"] - _cms).astype("float32")
    out["TyreOverdue"] = (out["TyreLife"] > _cms).astype(np.int8)
    out["NormTyre_x_RaceProgress"] = (out["NormTyreLife"] * out["RaceProgress"]).astype("float32")
