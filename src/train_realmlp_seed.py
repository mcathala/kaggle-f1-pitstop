"""Experiment 017 (cycle 5) — RealMLP multi-seed at one seed per invocation.

Identical recipe to experiment 016 except:
  - `SEED` env var controls RealMLP's `random_state` and torch.manual_seed.
  - The StratifiedKFold split seed stays 42 (CV protocol is frozen).
  - Outputs filename suffixed with the seed.

Run sequence for cycle 5:
  SEED=7   .venv/bin/python -u src/train_realmlp_seed.py
  SEED=99  .venv/bin/python -u src/train_realmlp_seed.py
  SEED=137 .venv/bin/python -u src/train_realmlp_seed.py
  SEED=313 .venv/bin/python -u src/train_realmlp_seed.py
  SEED=777 .venv/bin/python -u src/train_realmlp_seed.py

Then average all 6 OOFs (including cycle 4's seed 42 oof_realmlp.parquet) into
a multi-seed ensemble. Variance reduction without variant-selection — clean
generalization improvement, no public-LB-overfit risk.

Outputs:
  data/oof_realmlp_seed{SEED}.parquet
  data/submission_realmlp_seed{SEED}.csv
"""
...
ID_COL = "id"
N_SPLITS = 5
# Split seed stays 42 (CV protocol frozen). Only the RealMLP random_state
# varies per invocation.
SPLIT_SEED = 42
SEED = int(os.environ.get("SEED", "42"))
OOF_OUT = DATA / f"oof_realmlp_seed{SEED}.parquet"
SUB_OUT = DATA / f"submission_realmlp_seed{SEED}.csv"
